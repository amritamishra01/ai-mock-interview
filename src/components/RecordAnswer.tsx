...
import useSpeechToText from "react-hook-speech-to-text";
import type { ResultType as SpeechResultType } from "react-hook-speech-to-text";
...
const generateResult = async (
  ...
  try {
    const stream = await chatSession.sendMessage(prompt);
    ...
    const parsedResult: AIResponse = cleanJsonResponse(fullResponse);
    return parsedResult;
  } catch (error) {
    console.log(error);
    toast("Error", {
  ...
  useEffect(() => {
    ...
    .filter((result): result is SpeechResultType => typeof result !== "string")
